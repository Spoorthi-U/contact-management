...
function addContactToTable(contact) {
    ...
    deleteIcon.addEventListener('click', function () {
        ...
        deleteContactCallback = function () {
            col.remove(); // Remove the contact tile from the DOM

            // Assuming 'contact.phone' contains the phone number of the contact to delete
            ...
            fetch(`http://localhost:3000/api/contacts/${phoneNumber}`, {
                ...
                .then(response => {
                    if (response.ok) {
                        // If deletion is successful, show an alert or update UI
                        alert('Contact deleted successfully');
                    } else {
                        // If deletion failed (e.g., server returned an error status), handle it here
...
function enableEditMode(nameDiv, phoneDiv, genderSpan, emailSpan, groupSpan, streetSpan, citySpan, stateSpan, zipSpan, countrySpan, editIcon, saveIcon, img, fileInput) {
    ...
    streetSpan.contentEditable = true;
    citySpan.contentEditable = true;
    stateSpan.contentEditable = true;
    zipSpan.contentEditable = true;
    countrySpan.contentEditable = true;
...
function saveContactChanges(nameDiv, phoneDiv, genderSpan, emailSpan, groupSpan, streetSpan, citySpan, stateSpan, zipSpan, countrySpan, saveIcon, editIcon, img, fileInput) {
    ...
    streetSpan.contentEditable = false;
    citySpan.contentEditable = false;
    stateSpan.contentEditable = false;
    zipSpan.contentEditable = false;
    countrySpan.contentEditable = false;
    ...
    const updatedContact = {
        ...
        street: streetSpan.textContent.replace('Street: ', ''),
        city: citySpan.textContent.replace('City: ', ''),
        state: stateSpan.textContent.replace('State: ', ''),
        zip: zipSpan.textContent.replace('ZIP: ', ''),
        country: countrySpan.textContent.replace('Country: ', ''),
    ...
    fetch('http://localhost:3000/api/contacts', {
        ...
        .then(data => {
            if (data.error && data.error.sqlMessage && data.error.sqlMessage.includes('Duplicate entry')) {
                // Duplicate entry error for phone number
                alert('Phone number already exists for another contact');
            } else if (data.error) {
                // Other server-side errors
                alert(data.error); // Show the specific server-side error message
            } else {
                // Success case
